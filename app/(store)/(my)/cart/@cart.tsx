...
export default function CartList() {
  ...
  return (
    <div className="w-full flex gap-5 flex-col lg:flex-row">
        ...
        {items.length === 0 && (
          <div className="py-10">
            ...
            <p className="text-center max-w-[40ch] mx-auto p-10 text-sm">
              No hay productos en tu carrito, agrega algunos de nuestro{' '}
      ...
      {items.length > 0 && (
        <footer className="max-lg:border-t max-lg:px-7 min-w-[350px] border-dotted border-stone-500 max-lg:pt-5">
          ...
          <div className="flex py-1 text-sm border-t border-stone-700 pt-3 mt-3">
            <span className="flex flex-grow text-white">Subtotal:</span>
            <p>
          ...
          {isSignedIn ? (
            <button
              ...
              disabled={isLoading}
              className="w-full disabled:justify-center disabled:opacity-50 disabled:grayscale flex justify-between gap-2 items-center p-3 px-4 hover:bg-white/90 hover:scale-105 transition-all rounded-full bg-white text-black mt-5"
            >
              {isLoading ? (
                <>Procesando...</>
              ) : (
