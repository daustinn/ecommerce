    ...
    <ul className="flex flex-col gap-1 text-stone-50">
      {collections.map((collection) => (
        <li key={collection._id}>
          ...
          >
            <input
              checked={currentSearch === collection.slug?.current}
              type="checkbox"
              className="hidden peer"
            />
            <div className="w-3 group-hover:outline-blue-600 peer-checked:bg-blue-700 peer-checked:outline-blue-700 outline-stone-700 aspect-square rounded-sm outline-[1px] outline outline-offset-2"></div>
            {collection.title}
          </button>
        </li>
      ))}
